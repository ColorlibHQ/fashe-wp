...
class Epsilon_Welcome_Screen {
		...
		if ( isset( $_POST['args'], $_POST['args']['nonce'] ) && ! wp_verify_nonce( sanitize_key( $_POST['args']['nonce'] ), 'welcome_nonce' ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Not allowed', 'fashe' ),
					)
				)
			);
		}
		...
		if ( ! class_exists( $args_action[0] ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Class does not exist', 'fashe' ),
					)
				)
			);
		}
		...
		if ( is_admin() ) {
			...
			wp_localize_script(
				...
				array(
					...
					'no_required_actions_text' => esc_html__( 'Hooray! There are no required actions for you right now.', 'fashe' ),
					'ajax_nonce'               => wp_create_nonce( 'welcome_nonce' ),
					'activating_string'        => esc_html__( 'Activating', 'fashe' ),
					'body_class'               => 'appearance_page_' . $this->theme_slug . '-welcome',
					'no_actions'               => esc_html__( 'Hooray! There are no required actions for you right now.', 'fashe' ),
				)
			);
		}
	}
	...
	private function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= '<h1>' . sprintf( esc_html__( 'Welcome to %1$s', 'fashe' ), $this->theme_name ) . '</h1>';
			$this->notice .= '<p>';
			$this->notice .=
			...
			$this->notice .= '<p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome&tab=recommended-actions' ) ) . '" class="button button-primary button-hero" style="text-decoration: none;"> ' . sprintf( esc_html__( 'Get started with %1$s', 'fashe' ), $this->theme_name ) . '</a></p>';
	...
	public function welcome_screen_menu() {
		...
		$title = sprintf( esc_html__( 'About %1$s', 'fashe' ), esc_html( $this->theme_name ) );
	...
	private function set_default_sections( $config = array() ) {
		$arr = array(
			'getting-started'     => array(
				...
				'label' => __( 'Getting Started', 'fashe' ),
			...
			'recommended-actions' => array(
				...
				'label' => __( 'Recommended Actions', 'fashe' ),
	...
	private function check_plugin( $slug = '' ) {
		$arr = array(
			...
			'label'     => __( 'Install and Activate', 'fashe' ),
		);
