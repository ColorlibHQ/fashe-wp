...
class Epsilon_Section_Recommended_Actions extends WP_Customize_Section {
	...
	public function json() {
		...
		$json['facebook_text']  = esc_html__( 'Facebook', 'fashe' );
		$json['twitter']        = $this->twitter;
		$json['twitter_text']   = esc_html__( 'Twitter', 'fashe' );
		$json['wp_review']      = $this->wp_review;
		$json['wp_review_text'] = esc_html__( 'Review this theme on w.org', 'fashe' );
		if ( $this->wp_review ) {
			...
			if ( ! empty( $v['plugin_slug'] ) ) {
				...
				switch ( $active['needs'] ) {
					case 'install':
						...
						$v['button_label'] = esc_html__( 'Install', 'fashe' );
						break;
					case 'activate':
						$v['button_class'] = 'activate-now button button-primary';
						$v['button_label'] = esc_html__( 'Activate', 'fashe' );
						break;
					case 'update':
						$v['button_class'] = 'update-now button button-primary';
						$v['button_label'] = esc_html__( 'Update', 'fashe' );
						break;
					case 'deactivate':
						$v['button_class'] = 'deactivate-now button';
						$v['button_label'] = esc_html__( 'Deactivate', 'fashe' );
						break;
				}
		...
		foreach ( $this->plugins as $k => $v ) {
			...
			switch ( $active['needs'] ) {
				case 'install':
					...
					$t['button_label'] = esc_html__( 'Install', 'fashe' );
					break;
				case 'activate':
					$t['button_class'] = 'activate-now button button-primary';
					$t['button_label'] = esc_html__( 'Activate', 'fashe' );
					break;
				case 'update':
					$t['button_class'] = 'update-now button button-primary';
					$t['button_label'] = esc_html__( 'Update', 'fashe' );
					break;
				case 'deactivate':
					$t['button_class'] = 'deactivate-now button';
					$t['button_label'] = esc_html__( 'Deactivate', 'fashe' );
					break;
			}
